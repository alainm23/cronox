  ...
  async init() {
    ...
    if (this.platform.is('cordova')) {
      if (
        ...
        parseInt(this.device.version, 10) < 8
      ) {
  ...
  init_onesignal(usuario: any) {
    if (
      ...
      this.oneSignal.startInit(
        '822e4be3-ca69-472f-ba0a-7f7e8a84cfd6',
        '743541826445'
      );
      ...
      this.oneSignal
        ...
        .subscribe((OSNotification: any) => {
          // do something when notification is received
        });

      this.oneSignal
        .handleNotificationOpened()
        .subscribe((OSNotificationOpenedResult: any) => {
          const destino: string =
            OSNotificationOpenedResult.notification.payload.additionalData
              .destino;
          const clave: string =
            OSNotificationOpenedResult.notification.payload.additionalData
              .clave;
          ...
          if (destino === 'noticia') {
            this.navController.navigateForward(['noticia-detalle', clave]);
          } else if (destino === 'tramite') {
